...
public class Screen extends JPanel implements Runnable {
   ...
   private ScoreBoard scoreBoard;
    ...
    public Screen(Frame var1) {
      this.setLayout(null);
      this.mobNumber = level * 20; //at first 3
      this.mobSpawned = 0;
      this.spawnTime = 2000;
      this.spawnFrame = 0;
    ...
    pauseFrame.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
      ...
      startMenuButton.addActionListener(e -> {
          ...
          SwingUtilities.invokeLater(() -> {
              ...
              // For example, if you have a method called createAndShowGameFrame(), call it here
  ...
  public void closeCurrentGameFrame() {
   ...
   public void define() {
      ...
      for(int var2 = 0; var2 < tileset_groundGrass.length; ++var2) {
         ...
         tileset_groundGrass[var2] = this.createImage(new FilteredImageSource(tileset_groundGrass[var2].getSource(), new CropImageFilter(0, 25 * var2, 25, 25)));
      }
      ...
      for(int var3 = 0; var3 < tileset_air.length; ++var3) {
         ...
         tileset_air[var3] = this.createImage(new FilteredImageSource(tileset_air[var3].getSource(), new CropImageFilter(0, 25 * var3, 25, 25)));
      }
   ...
   public void paintComponent(Graphics var1) {
      ...
      var1.drawLine(room.block[0][0].x - 1, 0, room.block[0][0].x - 1, room.block[room.worldHeight - 1][0].y + room.blockSize);
      var1.drawLine(room.block[0][room.worldWidth - 1].x + room.blockSize, 0, room.block[0][room.worldWidth - 1].x + room.blockSize, room.block[room.worldHeight - 1][0].y + room.blockSize);
      var1.drawLine(room.block[0][0].x, room.block[room.worldHeight - 1][0].y + room.blockSize, room.block[0][room.worldWidth - 1].x + room.blockSize, room.block[room.worldHeight - 1][0].y + room.blockSize);
      room.draw(var1);
            ...
            for(int var1 = 0; var1 < mobs.length; ++var1) {
                if (!mobs[var1].inGame) {
                    ...
                    if (this.mobSpawned == this.mobNumber - 1) { // Ensure we leave space for the boss
                       ...
                       //System.out.println("Boss Spawned! Health: " + mobs[this.mobNumber].health + " Tower Damage: " + mobs[this.mobNumber].towerDamage + " Count: " + this.mobSpawned + " At Speed " + mobs[this.mobNumber].walkSpeed);
                    } else {

                        // Alternate between spawning mob1 and mobWarrior
                        if (this.mobSpawned % 2 == 0) {
                            mobs[var1].towerDamage = 10;
                            mobs[var1].spawnMob(Value.mobAngry, 1000 * level, 22 * level, 7, 22 / level); // First type of normal mob
                            //System.out.println("Normal Spawned! Health: " + mobs[var1].health + " Tower Damage: " + mobs[var1].towerDamage + " Count: " + this.mobSpawned + " At Speed " + mobs[var1].walkSpeed);
                        } else {
                            mobs[var1].towerDamage = 15;
                            mobs[var1].spawnMob(Value.mobWarrior, 1200 * level, 22 * level, 10, 16 / level); // Second type of normal mob (mobWarrior)
                            //System.out.println("Normal Spawned! Health: " + mobs[var1].health + " Tower Damage: " + mobs[var1].towerDamage + " Count: " + this.mobSpawned + " At Speed " + mobs[var1].walkSpeed);
                        }
